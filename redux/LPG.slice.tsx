...
export const chartAllRegionLPG = createAsyncThunk(
...
export const prices = createSlice({
  ...
  reducers: {
    ...
    setSixMonths: (state, action) => {
      state.six__months = action.payload;
    },
  },
  extraReducers: (builder) => {
    builder
      .addCase(chartAllRegionLPG.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(
        chartAllRegionLPG.fulfilled,
...
export const { setSelectedRegions, setSelectedState, setSixMonths } =
  prices.actions;
