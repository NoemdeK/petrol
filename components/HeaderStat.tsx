...
const HeaderStat = ({ data }: any) => {
  ...
  const renderProduct = (productType: any, data: any) => {
    ...
    return (
      <Link
        href={getLink(productType)}
        key={productType}
        className="p-2 bg-background border flex gap-2 rounded-md min-w-56 w-full cursor-pointer"
      >
        ...
        <div
          className={cn(
            "flex flex-col justify-between text-right ml-auto font-medium text-xs"
          )}
        >
          <p
            className={cn(isNegativeChange ? "text-red-600" : "text-green-600")}
          >
            {data.overallPricePercentChange}%
          </p>
          <p
            className={cn(
              isNegativeChangez ? "text-red-600" : "text-green-600"
            )}
          >
            {data.recentPricePercentChange}
          </p>
        </div>
      </Link>
    );
